...
class Aufgabe7 {
  public static void main(String[] args) {
    ...
    if (args.length != 3 || !args[1].toUpperCase().equals("TO")) {
      System.out.println("java EncryptBytes source to destination");
      return;
    }

    try {
      instr = new DataInputStream(new BufferedInputStream(new FileInputStream(args[0])));
      outstr = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(args[2])));

      try {
        int data;
        while (true) {
          data = instr.readUnsignedByte();
          // Apply bitwise complement for encryption
          data = ~data;
